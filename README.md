...
> A cross-provider, developer-first handbook for surviving Web Application Firewalls (WAFs) â€” Google Cloud Armor, AWS WAF, and Cloudflare â€” without disabling them.
...
1. **Legitimate traffic gets blocked** â€” long session cookies, JWTs, WYSIWYG HTML payloads, and signed URLs routinely trip rules like OWASP CRS `942100` (SQLi via `libinjection`), `941xxx` (XSS), or rate-limit rules.
2. **Developers cannot see it.** The request is denied at the edge. Your application container never executes. Logs are empty. The browser shows `403`, `404`, or `429`. The reflexive guess â€” "my code is broken" â€” is wrong, and the next hour is wasted.

This repository closes that loop with a **shared vocabulary**, **a 5-minute triage workflow**, and **safe, targeted tuning patterns** (exclusions and preview mode â€” never "disable the rule group").
...
| False-positive triage, target exclusions, preview/count mode rollouts     | Bot management product tuning (Cloudflare Bot Management, AWS Bot Control) â€” separate domain |
...
1. **The Symptom** â€” exactly what the user / browser / client sees.
2. **The Diagnosis** â€” the OWASP CRS rule ID and engine behavior (e.g. `libinjection`) responsible.
3. **The Log Evidence** â€” the precise log query needed to prove it, per cloud.
4. **The Remediation Matrix** â€” Console click-path **and** Terraform / CLI snippets for a **targeted exclusion** (never "disable the group").
5. **Audit trail entry** â€” what to record in [EXCEPTIONS.md](EXCEPTIONS.md) so the exception is reviewable later.
...
4. **Open a tuning request** using the template in [.github/PULL_REQUEST_TEMPLATE.md](.github/PULL_REQUEST_TEMPLATE.md) â€” include rule ID, path, redacted payload, and proposed exclusion scope.

---

## Contributing

False-positive reports, log-query corrections, and provider-parity additions are all welcome. Read [CONTRIBUTING.md](CONTRIBUTING.md) first â€” it includes the **payload redaction checklist** that every report must pass.
