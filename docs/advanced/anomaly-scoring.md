...
| After bumping CRS paranoia from PL1 → PL2, the FP rate spikes overnight even though no individual rule was changed | Deploy timeline |
| Two endpoints with identical rules behave differently — one is fine, the other 403s — because the second endpoint's typical request hits more low-scoring matches by chance | Per-endpoint stats |
...
Recap from [docs/rules/949110.md §2.1](../rules/949110.md#21-what-949110-does--the-anomaly-scoring-model): OWASP CRS in anomaly-scoring mode treats each match as a **score increment** rather than an immediate block. A separate "blocking evaluation" rule blocks at end-of-phase if the total score exceeds a threshold.
...
| **Paranoia level** | `tx.paranoia_level` (1–4, default `1`) | Which tranches of rules are enabled. Higher levels add more aggressive heuristics — more matches, more contributing scores. |
...
- Current: PL1, threshold 5. Average legit request hits 0–2 low-severity matches. ~0.1% FP rate.
- Bump to PL2 without changing threshold: ~20–40 more rules enabled, average legit request now hits 1–4 matches. Many push past threshold 5. FP rate rises to ~3–5%.
- Bump threshold to 7 along with PL2 bump: aggregate FP rate stays near baseline; new attack coverage is gained.

The pattern: **whenever you raise PL, simultaneously raise threshold by ~2–3 (more for higher PL bumps)**. Calibrate by running PL2 detection-only for a week, tallying the per-request score distribution, and choosing a threshold at the ~99.5 percentile of legitimate traffic scores.
...
| **Bot score** | Bot Management product (separate); `bot.score` in CEL | AWS WAF Bot Control rules contribute their own scores | `cf.bot_management.score` (1–99); separate from WAF anomaly score |
| **Reputation / IP-based** | Cloud Armor reputation lists | `AWSManagedRulesAmazonIpReputationList` (deny/count by reputation) | `cf.threat_score` (0–100) and `ip.src in $cf.tag.bad_reputation` |
...
Combining them in a rule is what produces "good but precise" policies. Example: deny only if **CRS anomaly score ≥ 5** AND **bot score < 30** AND **path is `/login`** — the intersection is narrow enough that FPs collapse.
...
| --- | --- | --- | --- | --- |
| **GCP Cloud Armor** | CRS via `inbound-anomaly-stable` | No (only matching rule IDs) | Via `sensitivity` 1–4 on `evaluatePreconfiguredWaf()` (maps to thresholds 5/7/10/15 roughly) | Implicit — `sensitivity` controls which rule tranches contribute |
| **AWS WAF** | Per-rule blocking; no aggregate score | N/A | N/A | N/A |
| **Cloudflare** | CRS via OWASP Core Ruleset + Cloudflare Managed Ruleset | Yes (in Security Events with payload logging) | Per ruleset under WAF managed rules config | Yes (PL1–PL4 selector in dashboard / Terraform) |

---

## 3. The Log Evidence

### 3.1 GCP Cloud Armor — per-rule contribution profiling
...
### 3.2 AWS WAF — surrogate-score by frequency
...
This identifies "groups of rules that frequently match together" — the AWS equivalent of "contributing rules to an aggregate".

### 3.3 Cloudflare — explicit score in events
...
```python
# score_reconstruct.py — feed it Cloud Armor log JSON, get cumulative scores.
import json, sys
...
> **Operating principle:** anomaly scoring is a **two-axis problem** — threshold setting and per-rule severity. Tune one axis at a time, measure for 7 days, then tune the other. Never bump paranoia + threshold simultaneously in production; the two changes interact in ways the FP rate won't show until 2–3 days later.

### 4.1 Threshold migration playbook

1. **Baseline (week 1).** Run a per-path score-distribution analysis (§3.1–3.3). Record the p50, p95, p99, p99.5, p99.9 scores.
2. **Target threshold (week 2).** Choose a threshold ~1 unit higher than the p99.5. This blocks the worst 0.5% of legitimate traffic — they will need exception scoping — and 100% of anything past it.
...
rule {
  ...
  description = "Effectively zero out 932130 contribution on /api/v1/notes (legitimate backticks in code snippets). Reduces anomaly score by 5 per matching request; brings sustained 949110 FPs to zero per §3.1 analysis. EXCEPTIONS.md row YYYY-MM-DD."
}
```

**Cloudflare:** managed-ruleset override sets the contributing rule's action to `log` on the scoped expression — score contribution stops on that path.
...
- Run a baseline + post-change score distribution diff (§3.1–3.3). Per-path p99 score should drop or stay flat; threshold breach rate should drop.
...
```
| 2026-06-17 | api.example.com | gcp-cloud-armor | anomaly threshold raised from sensitivity 1 → 2 on /api/v1/notes | path-scoped via evaluatePreconfiguredWaf('inbound-anomaly-stable', {sensitivity: 2}) | Score distribution: pre p99.5 = 5, post p99.5 = 7. Per §3.1 analysis, 932130 contributed +5 on legitimate code-snippet content. Compensating: app-layer validation in note handler (commit a1b2c3d) rejects exec/eval as content. | <link to before/after score histograms> | preview 2026-06-17 → enforce 2026-06-24 | @platform-lead + @secops-lead | 2026-12-14 |
```
...
- **AWS doesn't have this complexity, then customers try to simulate it.** Some teams build custom Lambda@Edge or CloudFront Functions to compose scores out of multiple AWS rule signals. This is technically possible but operationally expensive — consider whether you really need the model, or whether per-rule tuning is sufficient on AWS.
- **Adaptive Protection / ML-deployed rules interact with scoring.** GCP Adaptive Protection can auto-deploy rate-based rules during attacks. If your FP coincides with an Adaptive Protection event, the score isn't the culprit — disable the auto-deployed rule and re-evaluate.
- **Reproducibility across CRS package versions.** A CRS upstream version bump can rebalance severities (warning ↔ error) on existing rules. Score distributions shift overnight. Always run a re-baseline after CRS updates.

---

## See also

- [docs/rules/949110.md](../rules/949110.md) — the score-aggregator blocker rule itself; this page is the operating manual.
- [docs/rules/941100.md](../rules/941100.md), [docs/rules/942100.md](../rules/942100.md), [docs/rules/932100.md](../rules/932100.md) — common contributing rules.
- [docs/concepts/rate-limiting.md](../concepts/rate-limiting.md), [docs/concepts/geoblocking-exceptions.md](../concepts/geoblocking-exceptions.md) — neighboring decision systems frequently composed with anomaly scoring.
...
- Anomaly scoring concepts — [https://coreruleset.org/docs/concepts/anomaly_scoring/](https://coreruleset.org/docs/concepts/anomaly_scoring/).
- Paranoia levels — [https://coreruleset.org/docs/concepts/paranoia_levels/](https://coreruleset.org/docs/concepts/paranoia_levels/).
- `crs-setup.conf.example` (severity-score mapping) — [https://github.com/coreruleset/coreruleset/blob/main/crs-setup.conf.example](https://github.com/coreruleset/coreruleset/blob/main/crs-setup.conf.example).
- Rule `949110` (inbound blocker) and `980130` (outbound blocker) — [https://github.com/coreruleset/coreruleset/blob/main/rules/REQUEST-949-BLOCKING-EVALUATION.conf](https://github.com/coreruleset/coreruleset/blob/main/rules/REQUEST-949-BLOCKING-EVALUATION.conf), [https://github.com/coreruleset/coreruleset/blob/main/rules/RESPONSE-980-CORRELATION.conf](https://github.com/coreruleset/coreruleset/blob/main/rules/RESPONSE-980-CORRELATION.conf).

### Google Cloud Armor

- `evaluatePreconfiguredWaf` sensitivity tuning — [https://cloud.google.com/armor/docs/rule-tuning#tuning-by-sensitivity-level](https://cloud.google.com/armor/docs/rule-tuning#tuning-by-sensitivity-level).
- Bot Management (`bot.score`) — [https://cloud.google.com/armor/docs/bot-management](https://cloud.google.com/armor/docs/bot-management).
- Adaptive Protection — [https://cloud.google.com/armor/docs/adaptive-protection-overview](https://cloud.google.com/armor/docs/adaptive-protection-overview).

### AWS WAF

- WAF rule processing model — [https://docs.aws.amazon.com/waf/latest/developerguide/how-aws-waf-works.html](https://docs.aws.amazon.com/waf/latest/developerguide/how-aws-waf-works.html).
- `AWSManagedRulesAmazonIpReputationList` — [https://docs.aws.amazon.com/waf/latest/developerguide/aws-managed-rule-groups-list.html#aws-managed-rule-groups-list-ipreputation](https://docs.aws.amazon.com/waf/latest/developerguide/aws-managed-rule-groups-list.html#aws-managed-rule-groups-list-ipreputation).
- AWS WAF Bot Control — [https://docs.aws.amazon.com/waf/latest/developerguide/waf-bot-control.html](https://docs.aws.amazon.com/waf/latest/developerguide/waf-bot-control.html).

### Cloudflare

- OWASP Core Ruleset paranoia + threshold config — [https://developers.cloudflare.com/waf/managed-rules/reference/owasp-core-ruleset/](https://developers.cloudflare.com/waf/managed-rules/reference/owasp-core-ruleset/).
- Bot Management score (`cf.bot_management.score`) — [https://developers.cloudflare.com/bots/concepts/bot-score/](https://developers.cloudflare.com/bots/concepts/bot-score/).
- Threat score (`cf.threat_score`) — [https://developers.cloudflare.com/ruleset-engine/rules-language/fields/#field-cf-threat_score](https://developers.cloudflare.com/ruleset-engine/rules-language/fields/#field-cf-threat_score).
