# Cookie & JWT False Positives â€” OWASP CRS Rule `942100`
...
- The failure is **per-user**, not global â€” it correlates with users whose session cookie or JWT happens to contain certain byte sequences.
...
- `curl` without the `Cookie:` / `Authorization:` header returns `401` or `200` (depending on route) â€” the **presence of the credential** is what trips the block.
...
3. **Field type.** `942100` inspects **all `ARGS`** by default, including `request_cookies` and selected headers. Cookies are the highest-cardinality, highest-entropy field in most requests â€” they win the false-positive lottery.

### Sibling rules to be aware of

| Rule     | What it catches                          | Why it co-fires                                                        |
| -------- | ---------------------------------------- | ---------------------------------------------------------------------- | --- |
| `942100` | `libinjection` SQLi fingerprint          | The base rule â€” usually the terminating one.                           |
...
- `jsonPayload.enforcedSecurityPolicy.name` â€” which security policy triggered.
- `jsonPayload.enforcedSecurityPolicy.preconfiguredExprIds` â€” full list of CRS rules matched on this request (often includes `942130`, `942260` alongside `942100`).
- `httpRequest.requestUrl` â€” the path. Note it; the exclusion will be scoped to it.
- `httpRequest.userAgent`, `httpRequest.remoteIp` â€” sanity-check it's a real client, not a scanner.
...
Dashboard â†’ **Security â†’ Events** â†’ filter:
...
Each event row exposes the matched field under **Match details** when "Log full HTTP request" is enabled for the ruleset (it is **off** by default; enabling it has cost and privacy implications â€” confirm with SecOps first).
...
1. **Cloud Console â†’ Network Security â†’ Cloud Armor policies**.
2. Open the policy attached to your backend service.
3. **Rules â†’ + Add rule** â†’ **Mode: Preconfigured WAF rules**.
4. Expression: `evaluatePreconfiguredWaf('sqli-v33-stable', {'opt_out_rule_ids': ['owasp-crs-v030301-id942100-sqli']})` â€” **on a higher-priority rule scoped by path** (`request.path.matches('/login')`).
...
2. Edit the rule group `AWSManagedRulesCommonRuleSet` (or your equivalent SQLi group) â†’ **Rule group action overrides** â†’ set rule `SQLi_COOKIE` (or the specific `942100`-equivalent) to **Count** for the ACL.
...
`Count` is AWS's equivalent of preview mode. Promote to `none {}` override (full allow, scoped) once the count metric is stable and the alternative â€” adding an explicit allow with cookie inspection exclusion â€” has been reviewed.

### Cloudflare WAF

**Dashboard click-path:**

1. **Security â†’ WAF â†’ Managed rules â†’ Cloudflare Managed Ruleset â†’ Edit**.
2. Find rule `OWASP_CRS_V032_942100` (or the active CRS package equivalent).
3. **+ Add override** â†’ scope by **URI Path equals `/login`** â†’ **Action: Log** (Cloudflare's preview equivalent).
4. Save. Watch **Security â†’ Events** for 72h.
5. Once clean, change the override action from **Log** to **Skip** â€” and keep the URI path scope.
...
Cloudflare's managed rules do not expose per-cookie field exclusions in the same way AWS does; the standard pattern is **path scoping** + rule-level skip. If you need finer granularity, convert to a custom rule that runs `Skip â†’ Managed rules` for requests that match `http.cookie ~ "SESSIONID="` on `/login` only.

---

## 5. Audit Trail Entry

Append the following row to [../../EXCEPTIONS.md](../../EXCEPTIONS.md) the moment the exclusion lands in preview mode (not after enforce â€” preview hits matter):

```
| 2026-06-17 | app.example.com | gcp-cloud-armor | 942100 | request_cookies.SESSIONID on /login | Long signed session cookie tripping libinjection; verified harmless via reproduction in staging. | <link to log query result + curl trace> | preview 2026-06-17 â†’ enforce 2026-06-20 | @platform-lead | 2026-12-14 |
```

The **Review by** date defaults to +180 days. At that point, the exclusion gets re-validated against current traffic â€” if zero hits in preview mode without it, the row is struck through and the exclusion removed.

---

## Common pitfalls

- **Excluding the wrong field name.** Field-name matching is exact and case-sensitive on most providers. `Cookie` vs `cookie` vs `SESSIONID` vs `sessionid` will silently fail to match. Verify via a preview-mode hit count after applying â€” if hits stay at zero **and** the original `942100` denies persist, your exclusion is misnamed.
...
- **Forgetting the sibling rules.** After the exclusion, watch the next 72h of preview logs for `942130` / `942260` firing on the same field. If they do, extend the exclusion to those rule IDs **only** â€” never to the whole `942xxx` family.
...
- [../triage/README.md](../triage/README.md) â€” the 5-minute flow that leads here.
- [../provider-guides/gcp-cloud-armor.md](../provider-guides/gcp-cloud-armor.md) â€” full Cloud Armor reference: log fields, preview/enforce, opt-out vs allow patterns.
- [../provider-guides/aws-waf.md](../provider-guides/aws-waf.md) â€” full AWS WAF reference: scope-down statements, rule-action overrides, Count metric soak.
- [../provider-guides/cloudflare-waf.md](../provider-guides/cloudflare-waf.md) â€” full Cloudflare reference: managed rule overrides, custom skip rules.
- [../../EXCEPTIONS.md](../../EXCEPTIONS.md) â€” the ledger where the approved exclusion lives.
