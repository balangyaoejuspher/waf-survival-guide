# EXCEPTIONS â€” WAF Tuning Audit Trail
...
- Exclusions outlive the deploy that needed them â€” a JWT format change makes the exclusion redundant, but it stays in production forever.
...
| Evidence link | yes | Link to log query result or screenshot â€” proves the FP |
| Rollout mode | yes | `preview` first, then `enforce` â€” record the date each started |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| *example* `2026-06-17` | `app.example.com` | `gcp-cloud-armor` | `942100` | `request_cookies.SESSIONID` on `/login` | Long signed session cookie tripping `libinjection`; verified harmless. | (log query link) | `enforce` since `2026-06-20` (preview `2026-06-17 â†’ 2026-06-20`) | `@platform-lead` | `2026-12-14` |
...
- A removed exception is **not deleted from this file** â€” strike-through the row and append a "removed `YYYY-MM-DD`" note. The audit trail is append-only.
